...
class TenderApprovalsController extends ResourceController
{
    ...
    public function create()
    {
        ...
        // ✅ Check if the user has already voted on this tender
        $existingVote = $this->model->where([
            'user_id'   => $user_id,
            'tender_id' => $data['tender_id'],
        ])->first();

        if ($existingVote) {
            return $this->fail('You have already voted on this tender.');
        }
        ...
        if ($this->model->save($data)) {
            ...
            if ($newId) {
                ...
                $countApproved = $this->model
                    ->where(['approval_type' => 'approved', 'tender_id' => $data['tender_id']])
                    ->countAllResults();

                $countRejected = $this->model
                    ->where(['approval_type' => 'rejected', 'tender_id' => $data['tender_id']])
                    ->countAllResults();

                // ✅ If at least 3 approved votes, update status to "Approved"
                if ($countApproved >= 3) {
                ...
                // ✅ If at least 3 rejected votes, update status to "Rejected"
                if ($countRejected >= 3) {
                ...
                // ✅ Default success response if status not updated
                $response = [
                    "status"           => true,
